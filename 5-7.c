...
int readlines(char *linept[], char *line,int nlines);
...
int main()
{
    ...
    if ((nlines = readlines(linept,  lines, MAXLINES)) >= 0) {
...
int readlines(char *linept[], char *lines, int maxlines)
{
    int len, nlines, p;
    char *line;
    nlines = 0;
    ...
    while ((len = get_line(line, MAXLEN)) > 0)
        if (nlines >= maxlines || p + len + 1 > ALLOCSIZE)
            return -1;
        else {
            line[len] = '\0';
            strcp(line, line);
